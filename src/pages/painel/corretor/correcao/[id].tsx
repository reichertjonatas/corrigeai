    ...
    competenciasOffline.map((itemCompentencia: any, index: number) => {
      if (itemCompentencia.nota < 0) {
        ...
        if (itemCompentencia.obs.length <= 3) {
          errorMessage.push(
            `${itemCompentencia.title} - Preencher observação da competência`
          );
        }
      }
    });
                  ...
                  onClick={async () => {
                    if (currentDiscrepancia) {
                      ...
                      await strapi(session.jwt)
                        ...
                        .catch((error: any) => {
                          toast.warning(
                            "Atualize a página e tente novamente, code error: " +
                              error.message
                          );
                      ...
                      toast.error(
                        "É necessário selecionar uma correção discrepante!"
                      );
                    }
                  }}
                >
                  ENCERRAR DISCREPÂNCIA
                </span>
              </div>
            )}
          </div>
